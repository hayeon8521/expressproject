...
<template>
   <div>
      ...
      <button type="button"
         @click="setCount(num, $event)">Add {{ num }}</button> <!--$event 뷰가 제공하는 일종의 전역변수(이미등록되어있는 숨겨진변수)-->
...
<!--scoped 해당 컨포넌트에만 스타일을 적용하는 명령어-->
<!-- 컴포넌트 별로 스타일을 제한하고자 할때사용 하위에 적용되는 컴포넌트에 적용되지 않도록-->
<style scoped>
   form, form * {
      margin: 10px;
