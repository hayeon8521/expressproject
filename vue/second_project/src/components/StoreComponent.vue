...
<template>
    <div>
        <form>
            <label>
                제품ID : <input type="text" v-model="prodInfo.product_id">
            </label>
            <br>
            <label>
                제품명 : <input type="text" v-model="prodInfo.product_name">
            </label>
            <br>
            <label>
                카테고리 : 
...
export default{
    ...
    methods: {
        addCart(){
            //같은 객체여도 객체를 완전 끈어서 새로운 객체로 넘겨야함 참조타입오류 난다고함
            let obj = {
                product_id: this.prodInfo.product_id,
                product_name: this.prodInfo.product_name,
                category: this.prodInfo.category,
            };
            this.$store.dispatch('addProduct', obj);    //dispatch는 actions에 등록된 함수를 호출함
        },
    },
};
</script>